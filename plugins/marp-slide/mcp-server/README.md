...
ビルドは初回に必要です。Dockerfile・依存パッケージ・`src/`（ツール定義）のいずれかを更新した場合は再ビルドしてください。`src/index.ts` に追加された `marp_hash`・`validate_review_gate` はイメージに焼き込まれるため、古いイメージのままでは Stop hook やハッシュ取得が「存在しないツール」を呼びます。
...
MCP サーバーは 3 つのツールを公開します: `marp_export`（`source`／`format` (html/pdf/pptx/png)／`output` を指定してエクスポート）、`marp_hash`（`source` の SHA-256 を決定論的に計算。`review.json.source_sha256` 用）、`validate_review_gate`（Stop hook 用の完了ゲート判定）。手動での MCP 操作は不要です。
