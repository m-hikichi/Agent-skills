...
ビルドは初回に必要です。Dockerfile・依存パッケージ・`src/`（ツール定義）のいずれかを更新した場合は再ビルドしてください。
...
MCP サーバーが公開するツールは **`marp_export` の 1 つだけ**です（`source`／`format` (html/pdf/pptx/png)／`output` を指定してエクスポート）。手動での MCP 操作は不要です。
