...
26. `critical_review.status == "pass"`
27. `critical_review.audience_simulation.status == "pass"`
28. `critical_review.so_what_test.status == "pass"`
29. `critical_review.story_persuasiveness.status == "pass"`
30. `critical_review.visual_clarity.status == "pass"`
31. `critical_review.professional_quality.status == "pass"`
32. `critical_review.issues` が空配列である
33. `critical_review.exact_fix_instructions` が空配列である
34. `critical_review.structural_issues` が空配列である

### ユーザー承認

35. ワークフローにユーザー承認が含まれる場合、`request.yaml.approval.draft_approved == true`
