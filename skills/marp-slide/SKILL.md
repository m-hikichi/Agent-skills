...
スライド作成を始める前に、Marp CLIが使えるか確認する。

```bash
npx @marp-team/marp-cli --version
```

エラーになった場合は、ユーザーにインストールを提案する：

```bash
npm install -g @marp-team/marp-cli
```

Node.jsが入っていない場合は、まずNode.jsのインストールを案内する。
...
以下の3つが明確になったら、構成の設計に進んでよい：
1. **誰に**: 対象者とその前提知識レベルが具体的にわかっている
2. **何を**: スライドに盛り込むべき具体的な内容（トピック、データ、事例など）が揃っている
3. **なぜ**: このプレゼンのゴール（聞き手にどうなってほしいか）が明確になっている

逆に、この3つのどれかが曖昧なままスライドを作り始めると、的外れな資料になるリスクが高い。
...
Marpで使うにはフロントマターの `style` でWebフォントを読み込む。`--html` オプションが必要なので、エクスポート時に `npx @marp-team/marp-cli slide.md --html --pdf` のように指定する。
...
1. 構成案を提示する
2. ユーザーから「これでいい」「OKです」といった承認をもらう
3. 修正の要望があれば、構成案を修正して再度提示する
4. **承認を得てから**、Marp形式のMarkdownを書き始める
...
1. **ドラフトを作成して見せる**: 承認された構成に沿ってMarkdownを書く。この時点では完成度は70%でよい
2. **フィードバックを求める**: 「ここまでの内容で気になる点はありますか？」と具体的に聞く。漠然と「どうですか？」と聞くより、具体的なポイントを示すほうが有益なフィードバックが返ってくる
3. **フィードバックを反映して改善する**: ユーザーの指摘を受けて修正し、再度見せる
4. **納得いくまで繰り返す**: ユーザーが「これでいい」と言うまで、何度でもこのサイクルを回す

#### レビュー時のチェックポイント

ドラフトを提示するとき、以下の観点で自己チェックし、気になる点があればユーザーに確認する：

- この内容で、対象者は本当に理解できるか？
- 専門用語が対象者にとって難しすぎないか？逆に、噛み砕きすぎて冗長になっていないか？
- 「だから何？」と言われそうなスライドはないか？
- 情報の順番は自然か？前のスライドの内容を踏まえて次に進んでいるか？
- テキスト量が多すぎるスライドはないか？

### 10. エクスポート

Marp CLIを使ってスライドを出力する：
...
# 出力ファイル名を指定
npx @marp-team/marp-cli slide.md --pdf -o presentation.pdf
```

PDF出力にはChromiumが必要。初回実行時に自動ダウンロードされるが、時間がかかる場合がある旨をユーザーに伝える。
