---
name: spec-authoring
description: '`requirements.md` と `SPEC-XXX` を新規作成 / 更新し、`FC-` / `NF-` と `FR-` / `NFR-` の接続、`実装トレーサビリティ契約`、`実装完了条件`、`テスト仕様`、トレーサビリティマトリクス初期行まで整える authoring 専用 skill。記載は全て Claude が自動で行う。コード実装前に、仕様と要件を文章としてそろえたいときに使う。'
---

# Spec Authoring

この skill は「書く役」である。`requirements.md`、`SPEC-XXX`、トレーサビリティ接続を整え、authoring として必要な成果物をそろえる。**記載は全て Claude が自動で行い、人間に手動作業を要求しない。** コード実装、最終整合性監査、次工程の制御はここに持ち込まない。
...
- トレーサビリティマトリクスの初期行追加または更新（Claude が自動記載）
- `SPEC-XXX` の `status` フィールドを適切に設定する（`draft` → `review`）
...
- **`status` は新規作成時に `draft` を設定し、内容が固まったら `review` に変更する。** `approved` への変更はユーザーの承認を得てから行う。
...
- `テスト仕様` では、少なくとも正常系、異常系、境界値、権限 / 状態遷移の観点を漏らさない。**各 `FR-` / `NFR-` に対して最低でも正常系・異常系・境界値の 3 種別のテストケースを定義する。**

### トレーサビリティマトリクス

- `requirements.md` と `SPEC-XXX` に書いた接続と同じ粒度で初期行を **Claude が自動で追加する**。
...
2. [spec-template.md](references/spec-template.md) を参照して `SPEC-XXX` を新規作成または更新し、`対応要件`、`機能仕様`、`実装トレーサビリティ契約`、`実装完了条件`、`テスト仕様` を埋める。**`status` を `draft` または `review` に設定する。**
3. 実装前でも、予定している実装ファイル、シンボル名、テストIDを `実装トレーサビリティ契約` に置く。
4. [traceability-matrix-template.md](references/traceability-matrix-template.md) を参照して、トレーサビリティマトリクスに対象要件と対象仕様の初期行を **Claude が自動で追加または更新する**。
...
- **`テスト仕様` の各 `FR-` / `NFR-` に正常系・異常系・境界値の 3 種別が最低 1 件ずつある**
- `FC-` / `NF-` と `FR-` / `NFR-` の対応が追える
- **`SPEC-XXX` の `status` が `draft` または `review` に設定されている**
- マトリクス初期行が追加または更新されている（Claude が自動記載済み）
