...
## 最終整合性監査の組み立て（必須 — これがないと Stop hook が block する）

CLOSE_OUT フェーズで、`main/references/final-audit-template.md` に従って `## 最終整合性監査` ブロックを**最終メッセージに必ず含める**。このセクションなしに停止すると Stop hook が即座に block を返し、作業に戻される。
...
1. `## 最終整合性監査` ブロック（`判定: clean` or `判定: needs-user-decision`）を**最終メッセージに含める** — 省略不可
2. auditor に `shutdown_request` を送る
3. 停止する → Stop hook (`review_stop_gate.py`) が整合性チェック + テスト + 監査フォーマットを検証

※ サブエージェント（auditor 等）の停止には軽量チェック（整合性のみ）が適用される。最終整合性監査セクションを書くのは orchestrator だけの責務。
