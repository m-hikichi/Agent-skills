...
skills/
  <skill-name>/
    README.md        # セットアップ手順
    .claude/         # プロジェクトルートにコピーする配布ディレクトリ
      skills/<skill-name>/
        SKILL.md     # スキル本体
      agents/        # 任意: subagent 定義
      settings.json  # 任意: hooks 設定
    assets/          # 任意: 画像・テンプレートなど
    scripts/         # 任意: 実行スクリプト
    references/      # 任意: 参照資料
```

各スキルの `.claude/` フォルダは、利用先プロジェクトのルートにそのままコピーすることで導入できます。詳しい手順は各スキルの `README.md` を参照してください。
...
- `marp-slide`: Marpを使ったスライド作成・レビュー・エクスポート支援（subagent・hooks付き）
