...
├── external/
│   └── drawio-mcp/                   # draw.io 公式 Skill / MCP Server 実装集（submodule）
├── docs/
│   └── claude-code/                  # Claude Code 手順書・Tips
├── .gitmodules                       # submodule 定義
└── README.md
```

各 Plugin / Skill / Hook / 外部リポジトリの内部構成・セットアップ・使い方は、それぞれの README を参照してください。

## Submodule の初期化

このリポジトリには submodule が含まれます。clone 時にまとめて取得する場合は、以下を実行してください。

```bash
git clone --recurse-submodules https://github.com/m-hikichi/Agent-skills.git
```

すでに clone 済みで submodule の中身が空に見える場合は、以下で初期化・取得できます。

```bash
git submodule update --init --recursive
```

submodule の追加・更新後は、必要に応じて以下で登録先の内容を同期してください。
...
1. 新規追加は実体に応じて `skills/`、`plugins/`、`hooks/`、`external/` に分類する
2. 変更は小さく分けてコミットする
...
- [`drawio-mcp`](external/drawio-mcp/README.md): draw.io 公式の Skill / MCP Server 実装集。LLM から draw.io 図を作成・編集するための MCP App Server / MCP Tool Server / Skill + CLI などを参照するための submodule。

### Docs (`docs/`)

- [`ClaudeCode_manual.md`](docs/claude-code/ClaudeCode_manual.md): Claude Code の基本操作・設定・連携手順のメモ。
- [`ClaudeCode_advanced-tips.md`](docs/claude-code/ClaudeCode_advanced-tips.md): Claude Code の応用的な使い方や運用 Tips。
